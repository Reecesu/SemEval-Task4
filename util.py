...
def micro_f1_score(predicted_labels: List[int], true_labels: List[int], classes: List[int]):
    """
    Calculate the micro f1-score for each class and return the average of it
    ...
    FN = 0
    for c in classes:
        TP += t_sum(logical_and(np.array([pred == c for pred in predicted_labels]), np.array([lab == c for lab in true_labels])))
        FP += t_sum(np.array([pred == c for pred in predicted_labels])) - TP
        FN += t_sum(np.array([lab == c for lab in true_labels])) - TP
    if TP + FP + FN:
